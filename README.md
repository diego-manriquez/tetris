...
- Una **pieza tuerca** de reto: un bloque 3 × 3 con un agujero circular en el centro que aparece de forma esporádica (~10 % de las piezas). El hueco es real, así que cualquier fila que lo contenga **no se limpia** hasta que otra pieza consiga rellenar ese agujero.
...
- **Modelo del tablero**: una matriz `ROWS × COLS` donde cada celda guarda `0` (vacía), un índice de color (1–8) que identifica la pieza, o `-1` (`HOLE`) para el agujero de una tuerca ya fijada: se dibuja pero no cuenta como celda llena.
...
| `NUT_CHANCE`   | Probabilidad de que la pieza sea la tuerca | `0.1`               |
